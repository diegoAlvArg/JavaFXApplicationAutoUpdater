...
//	public static final String TUTORIALS = "https://www.youtube.com/playlist?list=PL-xqaiRUr_iRKDkpFWPfSRFmJvHSr1VJI";
    ...
    public static final String getBasePathForClass(Class<?> classs) {
        ...
        try {
            ...
            LogRecord logRegistro = new LogRecord(Level.WARNING, String.format(Main.getResourceBundle().getString(ResourceLeng.TRACE_ERROR_URI), 1)
                    + errors.toString());
            logRegistro.setSourceClassName("getBasePathForClass");
...
//            Logger.getLogger(classs.getName()).log(Level.WARNING, "Cannot firgue out base path for class with way (1): ", ex);
        ...
        if (failed) {
            try {
                ...
                StringWriter errors = new StringWriter();
                ex.printStackTrace(new PrintWriter(errors));
                LogRecord logRegistro = new LogRecord(Level.WARNING, String.format(Main.getResourceBundle().getString(ResourceLeng.TRACE_ERROR_URI), 2)
                        + errors.toString());
                logRegistro.setSourceClassName("getBasePathForClass");
                LoggGen.log(logRegistro);
