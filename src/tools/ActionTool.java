    ...
    public static boolean openWebSite(String uri) {
        ...
        try {
            ...
            if (!Desktop.isDesktopSupported()) {
                ActionTool.showNotification(rb.getString(ResourceLeng.WEB_ERROR_TITLE), rb.getString(ResourceLeng.WEB_ERROR_MSJ) + uri + " ]", Duration.millis(2500), NotificationType.INFORMATION);
//                ActionTool.showNotification("Problem Occured", "Can't open default web browser at:\n[" + uri + " ]", Duration.millis(2500), NotificationType.INFORMATION);
            ...
            ActionTool.showNotification(rb.getString(ResourceLeng.WEB_OK_TITLE), rb.getString(ResourceLeng.WEB_OK_MSJ) + uri, Duration.millis(1500), NotificationType.INFORMATION);
...
//            ActionTool.showNotification("Problem Occured", "Can't open default web browser at:\n[" + uri + " ]", Duration.millis(2500), NotificationType.INFORMATION);
            ...
            LogRecord logRegistro = new LogRecord(Level.WARNING, Main.getResourceBundle().getString(ResourceLeng.TRACE_ERROR_ONWEB) + errors.toString());
            logRegistro.setSourceClassName("openWebSite");
...
//		System.out.println("Copying ->" + source + "\n\tto ->" + destination);
        try {
            ...
            StringWriter errors = new StringWriter();
            ex.printStackTrace(new PrintWriter(errors));
            LogRecord logRegistro = new LogRecord(Level.WARNING, Main.getResourceBundle().getString(ResourceLeng.TRACE_ERROR_ONCOPY) + errors.toString());
            logRegistro.setSourceClassName("copy");
            LoggGen.log(logRegistro);
//            logger.log(Level.WARNING, "", ex);
    ...
    public static boolean doQuestion(String text, Stage window) {
        ...
        alert.setHeaderText(Main.getResourceBundle().getString(ResourceLeng.QUESTION));
